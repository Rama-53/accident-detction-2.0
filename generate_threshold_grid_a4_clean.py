...
# Using a bit less height for the actual plots to easily fit the large global legend below
fig, axes = plt.subplots(2, 2, figsize=(A4_WIDTH, A4_HEIGHT * 0.75)) 
...
# Keep track of lines for the global legend
lines_for_legend = []
labels_for_legend = []
...
def plot_quadrant(ax, data_keras, data_resnet, data_hybrid, title, ylabel, ylim):
    ...
    l2, = ax.plot(thresholds, data_resnet, marker='^', linestyle='-.', color=c_resnet, linewidth=2.0, markersize=6, label='Secondary (ResNet)')
    ...
    # We DO NOT plot the legend inside the quadrant anymore to prevent overlaps!
    return [l1, l2, l3], ['Primary (Keras)', 'Secondary (ResNet)', 'Hybrid System']

# Plot the 4 metrics and collect the line objects for the global legend
...
# Add the target line and optimal star explicitly to the global legend list
l_target = axes[0,0].axhline(y=90, color='gray', linestyle='--', alpha=0.5, label='Target: 90%')
l_star, = axes[1, 1].plot(0.7, 88.9, 'r*', markersize=12, label='Optimal (0.7, 88.9%)')

lines.extend([l_target, l_star])
labels.extend(['Target: 90%', 'Optimal (0.7, 88.9%)'])

# Create a massive GLOBAL LEGEND at the very bottom of the entire image, entirely outside the graphs
fig.legend(lines, labels, loc='lower center', bbox_to_anchor=(0.5, 0.16), ncol=3, fontsize=10, frameon=True)

# Math terms box underneath the legend
...
# Widen the horizontal space, and adjust rect so the bottom 25% of the paper is reserved strictly for legends
plt.subplots_adjust(wspace=0.35, hspace=0.3)
plt.tight_layout(rect=[0.02, 0.25, 0.98, 0.94]) 

plt.savefig('performance_threshold_a4_noverlap.png', dpi=300, bbox_inches='tight')
print("Graph saved without overlaps to performance_threshold_a4_noverlap.png")
